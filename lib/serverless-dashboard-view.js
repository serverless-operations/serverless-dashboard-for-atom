...
export default class ServerlessDashboardView {
  ...
  update (newProperties) {
    if (typeof newProperties.consoleOutput !== 'undefined') {
  ...
  slsDeploy () {
    ...
    this.properties.consoleOutput = '$ serverless deploy<br/><br/>'
    this.action.deploy().stdout.on('data', (data) => {
      this.update({
  ...
  slsRemove () {
    this.resetCosnoleOutput()
    this.properties.consoleOutput = '$ serverless remove<br/><br/>'
    this.action.remove().stdout.on('data', (data) => {
      this.update({
        consoleOutput: `${this.properties.consoleOutput}${data.toString()}`
      })
    })
  ...
  slsAction (action, functionName) {
    ...
    switch (action) {
      case 'deploy-function':
        this.properties.consoleOutput = `$ serverless deploy function -f ${functionName}<br/><br/>`
        break
      case 'invoke':
        this.properties.consoleOutput = `$ serverless invoke -f ${functionName}<br/><br/>`
        break
      case 'logs':
        this.properties.consoleOutput = `$ serverless logs -f ${functionName}<br/><br/>`
        break
    }

    this.action.action(action, functionName).stdout.on('data', (data) => {
      this.update({
        consoleOutput: `${this.properties.consoleOutput}${data.toString()}`
      })
    })
